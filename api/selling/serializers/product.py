...
class ProductsSerializer(ModelSerializer, ProductImagesSerializer):
    sizes = SizingSerializer(many=True, required=False)

    class Meta:
        model = Product
        fields = ('id', 'name', 'thumbnail_url', 'images', 'price', 'tags', 'sizes', 'trimmed_description')


class ProductSerializer(ModelSerializer, ProductImagesSerializer):
    sizes = SizingSerializer(many=True, required=False)

    class Meta:
        model = Product
        fields = ('id', 'name', 'images', 'description', 'price', 'published', 'tags', 'sizes')


class ProductCreateUpdateSerializer(ModelSerializer, ProductImagesSerializer):
    sizes = SizingSerializer(many=True, required=False)
